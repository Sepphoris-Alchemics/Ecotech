...
namespace Ecotech
{
    public abstract class ThingComp_AutoHaulThingContainer : CompThingContainer
    {
        ...
        protected virtual bool HaulThingValidator(Thing haulThing, Pawn hauler) => hauler.CanReserve(haulThing);
        ...
        public virtual Thing FindHaulThingFor(Pawn pawn)
        {
            ...
            Thing foundThing = GenClosest.ClosestThingReachable(pawn.Position, pawn.Map, ThingRequest, PathEndMode.ClosestTouch, traverseParms, validator: (Thing thing) => HaulThingValidator(thing, pawn));
            return foundThing;
        }
    }
}
